    ...
    async _call(images, {
        ...
        if (!subtask) {
            ...
            for (let i = 0; i < imageSizes.length; ++i) {
                ...
                // In the event that the mask is a square, we should resize to
                // the largest length and center crop.
                const item_image = RawImage.fromTensor(item.mul_(255).to('uint8'));
                let new_width = null
                let new_height = null;
                if (item_image.width === item_image.height && size[0] !== size[1]) {
                    const max_size = Math.max(size[0], size[1]);
                ...
                const mask = await item_image.resize(new_width, new_height)
                    .then(mask => mask.center_crop(size[0], size[1]));
